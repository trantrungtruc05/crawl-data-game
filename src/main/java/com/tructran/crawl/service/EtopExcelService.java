...
public class EtopExcelService {
    ...
    public List<ExcelFileEntity> generateExcelList(String type) {

        try{

            List<CompareResultEntity>  compareResultEntityList = etopfunPageRepository.getResult(type);
            ...
            for(CompareResultEntity compareResultEntity : compareResultEntityList){
                ...
                excelFileEntity.setSlot(compareResultEntity.getSlot());
                excelFileEntity.setCategory(compareResultEntity.getCategory());
    ...
    public List<ExcelFileEntity> generateExcelListFilter(int min, int max) {
        ...
        try{
            ...
            List<CompareResultEntity>  compareResultEntityList = etopfunPageRepository.getResult("page");
            List<ExcelFileEntity> excelFileEntityList = new ArrayList<>();
    ...
    public void writeHeader(XSSFSheet sheet){
        ...
        headerCell.setCellStyle(cellStyle);
        headerCell.setCellValue("% different Empire vs Etop");
    ...
    public void writeData(XSSFWorkbook workbook, XSSFSheet sheet, String type){

        int rowCount = 1;

        List<ExcelFileEntity> excelFileEntityList = this.generateExcelList(type);
        for(ExcelFileEntity excelFileEntity : excelFileEntityList){
            ...
            cell = row.createCell(columnCount++);
            cell.setCellValue(excelFileEntity.getSlot());

            cell = row.createCell(columnCount++);
            cell.setCellValue(excelFileEntity.getCategory());
        }
